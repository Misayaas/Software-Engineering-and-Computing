...
//----------------------------------------------------------------------
    ...
    public void setup(LineStorage lines) {
        ...
        for(int i = 0; i < lines.getLineCount(); i++){
            for(int j = 0; j < lines.getWordCount(i); j++){
                String[] shiftWords = new String[lines.getWordCount(i)];
                for(int k = 0; k < shiftWords.length; k++){
                    shiftWords[k] = lines.getWord((j + k) % shiftWords.length, i);
                }
                shifts_.addLine(shiftWords);
            }
